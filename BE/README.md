...
> Crispin <br>
```text
정말 많은걸 배우고 적용시켜본 프로젝트였다. 지금 생각해보면 이 프로젝트를 이 정도 까지 발전시켰다는게 참 신기하다.

시간으 오래걸렸지만 그 만큼 많은걸 배울수 있었고 프론트 엔드와 협업을 통해 조금이나마 협업 하는 방법에 대해 알 수 있었다.

혼자 프로젝트를 진행하는것보다 여럿이 진행하는것이 생각보다 쉽지 않다는 것도 느꼈고 함께 작업할수 있는 동료가 있다는게 참 든든하다는 것도 느낄수 있었다.

프로젝트 시작부터 여러 문제가 발생하였는데, 그 문제를 해결할때마다 문서화를 했다면 더 좋았을것 같다는 아쉬움이 있다.

정말 빈번하게 발생하는 문제이지만 N + 1 문제 해결을 통해 조금이나마 성능 개선에 대해 알게되어 좋았다.
```
